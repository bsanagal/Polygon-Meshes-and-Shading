...
public class PolyMesh extends Shape
{
    ...
    private void computeAllNormals()
    {
        ...
        for(i=0;i<vertices.length;i++){
            ...
            v = (VertexPM) vertices[i]; //copy vertices obj reference
            v.setNormal(new Vector3d(0,0,0)); //Navg = 0
            
         }
        for(i=0;i<polygons.length;i++){ //for each polygon
            for(j=0;j<polygons[i].size();j++){ // for each vertex in polygon
                VertexPM v1 = new VertexPM();
                v1 = (VertexPM) polygons[i].getVertex(j);
                Vector3d normavg = new Vector3d();
                normavg.add(polygons[i].getVertex(j).getNormal(),
                        polygons[i].getNormal()); //navg += npolygon
                v1.setNormal(normavg);
        ...
        for(i=0;i<polygons.length;i++){
            polygons[i].getNormal().normalize();
        }
